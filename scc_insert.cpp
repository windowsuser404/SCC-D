...
void insert_condense(graph& g, int*& scc_maps, int num_verts, int edges, int*& scc_outarr, int*& scc_inarr, unsigned int*& scc_outdegree_list, unsigned int*& scc_indegree_list, int& roots, int& C_edges, int*& scc_index, int*& inverse_scc){
	...
	inverse_scc = new int[num_verts]; //inverse_scc[scc_root_node] = vertex in condensed graphs
	int root_count = 0,i;
	...
	new_create_csr(root_count, condensed_edges, scc_src, scc_dst, scc_outarr, scc_inarr, scc_outdegree_list, scc_indegree_list);
#if DEBUG
	printf("\n\ninarray at %p\n",&scc_inarr);
...
void condense_scc_update(graph& g, int* scc_maps, int* scc_index, int* inverse_scc, int* scc_diff_out, int* scc_diff_in, unsigned int* scc_diff_outdeg_list, unsigned int* scc_diff_indeg_list){
